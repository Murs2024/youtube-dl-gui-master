    ...
    public override void LoadLanguage() {
        if (BatchDownload) {
            ...
            if (MediaDetails?.InfoRetrieved == true) {
                ...
                this.Text = Language.frmExtendedDownloaderRetrieving.Format(Language.ApplicationDisplayName);
    ...
    private void BeginDownload(bool Auth) {
        ...
        ProcessingThread = new(() => {
            ...
            if (Status != DownloadStatus.Aborted && Status != DownloadStatus.AbortForClose) {
                ...
                else if (DownloadHelper.IsProcessUserInterruptExit(exitCode)) {
                    Status = DownloadStatus.Aborted;
                }
                else {
                    Status = DownloadStatus.YtdlError;
                }
            }
    ...
    private void BeginBatchDownload() {
        ...
        ProcessingThread = new(() => {
            ...
            for (int i = 0; i < lvQueuedMedia.Items.Count; i++) {
                ...
                if (Status != DownloadStatus.Aborted && Status != DownloadStatus.AbortForClose) {
                    int batchExit = DownloadProcess.ExitCode;
                    int batchIcon = batchExit == 0 ? StatusIcon.Finished
                        : DownloadHelper.IsProcessUserInterruptExit(batchExit) ? StatusIcon.Waiting
                        : StatusIcon.Errored;
                    lvQueuedMedia.Invoke(() => lvQueuedMedia.Items[i].ImageIndex = batchIcon);
                }
                else {
