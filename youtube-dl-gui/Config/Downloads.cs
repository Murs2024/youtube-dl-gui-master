...
internal static class Downloads {
    ...
    /// <summary>
    /// Папка <c>YouTubeDownloads</c> в корне репозитория (три уровня вверх от <c>bin\Debug|Release</c>).
    /// </summary>
    internal static string DefaultDownloadFolderNextToProject {
        get {
            string? exeDir = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            if (exeDir.IsNullEmptyWhitespace()) {
                exeDir = Environment.CurrentDirectory;
            }
            return System.IO.Path.GetFullPath(System.IO.Path.Combine(exeDir, "..", "..", "..", "YouTubeDownloads"));
        }
    }
    ...
    static Downloads() {
        ...
        fdownloadPath =
            IniProvider.Read(downloadPath, DefaultDownloadFolderNextToProject, ConfigName);

        try {
            System.IO.Directory.CreateDirectory(fdownloadPath);
        }
        catch {
            /* путь из ini может быть недоступен — запуск не блокируем */
        }
